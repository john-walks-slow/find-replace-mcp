...
import { ENCODING_HINT } from './types.js';
import { sha256 } from './utils.js';
const UTF8_DECODER = new TextDecoder('utf-8', { fatal: true });
export function detectEncoding(raw, requested) {
    if (requested) {
        return {
            encoding: requested,
            source: 'explicit',
...
function detectBomDetails(raw, requested) {
    const bom = detectBom(raw);
...
function encodingCodecName(encoding) {
    switch (encoding) {
        case 'utf-8':
            return 'utf8';
        case 'utf-16le':
        ...
        case 'shift_jis':
            return 'shift_jis';
        default: {
            const exhaustive = encoding;
            return exhaustive;
        }
    }
}
export function stripBom(raw, detected) {
    return detected.bomBytes > 0 ? raw.subarray(detected.bomBytes) : raw;
}
export function decodeTextFile(raw, detected) {
    return iconv.decode(stripBom(raw, detected), encodingCodecName(detected.encoding));
}
export function canEncodeText(content, encoding) {
...
export function encodeTextFile(content, detected) {
...
export function unsupportedEncodingMessage(filePath, requested) {
...
export async function tryReadTextFile(absolutePath, filePath, requestedEncoding, strict = false) {
...
export async function readTextFileStrict(absolutePath, filePath, requestedEncoding, strict = true) {
