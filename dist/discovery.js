...
export async function discoverCandidateFiles(config) {
    if (config.filePath) {
        return [
            {
    ...
    const files = [];
    const stack = [{ directory: config.basePath, inheritedIgnores: [] }];
...
function isIgnoredByGitignore(ignoreStack, absolutePath, isDirectory) {
...
function matchesIncludeExclude(filePath, include, exclude) {
...
function parseMaxFileBytes(value) {
