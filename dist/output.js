...
export function serializeMatch(match, includeReplacement) {
    return {
        id: match.id,
...
export function buildFileSummaries(matches) {
...
export function buildSearchSummary(result, options = {}) {
    return {
        query: result.query,
...
export function buildFindNextStep() {
    return {
...
export function buildPrepareNextStep(status, sessionId) {
...
export function buildSessionNextStep(session) {
...
function formatScope(result) {
    const parts = [result.mode, `encoding=${result.resultEncoding}`];
    if (result.wholeWord) {
...
function formatMatchLine(match, includeReplacement) {
...
function formatFileSummaryLines(matches, limit = 8) {
...
function formatMatchLines(matches, includeReplacement, limit = 12) {
...
export function formatFindText(result) {
    const lines = [
...
export function formatPrepareText(result, status, sessionId, expiresAt) {
    const lines = [
...
export function formatInspectSessionText(session) {
    const lines = [
...
export function formatApplyText(sessionId, changedFiles, replacementsApplied, createBackup) {
    const lines = [
