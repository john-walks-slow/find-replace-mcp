...
export async function executeSearch(config, query, replacement) {
    ...
    const limitation = limitationCode === 'apply_via_session_only'
...
export async function ensureFilesUnchanged(session) {
    for (const [filePath, fingerprint] of Object.entries(session.fileFingerprints)) {
        ...
        if (current.encoding !== fingerprint.encoding ||
...
function resolveResultEncoding(config, fileFingerprints) {
...
function resolveEncodingSource(config, fileFingerprints) {
    if (config.requestedEncoding) {
        return 'explicit';
    }
