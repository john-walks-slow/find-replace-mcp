...
import { buildFileSummaries, buildPrepareNextStep, buildSearchSummary, buildSessionNextStep, buildSessionSummary, fail, formatApplyText, formatFindText, formatInspectSessionText, formatPrepareText, ok, serializeMatch, serializeSearchResult } from './output.js';
import { ensureFilesUnchanged, executeSearch } from './search.js';
import { groupByFile, selectMatches, SessionStore } from './sessions.js';
import { ENCODING_HINT, SESSION_TTL_MS, SUPPORTED_ENCODINGS } from './types.js';
...
export function createServer() {
    const server = new McpServer({ name: 'smart-find-replace-mcp', version: '0.7.2' }, {
        instructions: [
    ...
    const resolveDefaultBasePath = async () => {
        try {
    ...
    server.registerTool('find_in_files', {
        title: 'Find In Files',
        description: 'Search under basePath by default, or restrict to one file with filePath. include/exclude are relative to basePath, and useGitIgnore controls whether ignored files are skipped.',
    ...
    server.registerTool('prepare_replace_in_files', {
    ...
    server.registerTool('inspect_replace_session', {
    ...
    server.registerTool('apply_replace_in_files', {
        ...
        try {
            ...
            const selectedMatches = selectMatches(session, selectionMode, matchIds);
